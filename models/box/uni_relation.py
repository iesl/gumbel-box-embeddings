...
class UniRelationModel(BaseBoxModel):
    ...
    def get_ranks(self, embeddings: Dict[str, torch.tensor]) -> Any:
        ...
        with torch.no_grad():
            b = embeddings
            intersection_prob = b['node'].intersection_log_soft_volume(
                self.entity(torch.arange(self.num_entities).unsqueeze(0)), temp=self.softbox_temp)  

            score_parent = (intersection_prob - self.entity(
                torch.arange(self.num_entities)).log_soft_volume(temp=self.softbox_temp)).reshape(-1)
            score_child = (intersection_prob - b['node'].log_soft_volume(temp=self.softbox_temp)).reshape(-1)

            parent_set = torch.zeros(self.num_entities)
            child_set = torch.zeros(self.num_entities)

            parent = (torch.where(
                score_parent > -0.6, score_parent, parent_set)).nonzero().reshape(-1)
            child = (torch.where(
                score_child > -0.6, score_child, child_set)).nonzero().reshape(-1)
...
@Model.register('uni-relation-model-conditional-training')
